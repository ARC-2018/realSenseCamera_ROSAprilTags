...
AprilTagDetector::AprilTagDetector(ros::NodeHandle& nh, ros::NodeHandle& pnh): it_(nh){
  ...
  AT_pub_ = nh.advertise<geometry_msgs::PoseArray>("AprilTagPointsRaw",1);
}
...
void AprilTagDetector::imageCb(const sensor_msgs::ImageConstPtr& msg, const sensor_msgs::CameraInfoConstPtr& cam_info){
  ...
  geometry_msgs::PoseArray AT_poses;
  AT_poses.header = cv_ptr->header;
  ...
  BOOST_FOREACH(AprilTags::TagDetection detection, detections){
    ...
    geometry_msgs::PoseStamped msgPoints;
    for(int q=0;q!=4;q++){
      std::pair<float,float> currP = detection.p[q];
      msgPoints.pose.position.x = currP.first;
      msgPoints.pose.position.y = currP.second;
      AT_poses.poses.push_back(msgPoints.pose);
    }
    msgPoints.pose.position.x = detection.cxy.first;
    msgPoints.pose.position.y = detection.cxy.second;
    AT_poses.poses.push_back(msgPoints.pose);
